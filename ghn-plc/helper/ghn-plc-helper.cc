...
TypeId GhnPlcHelper::GetTypeId(void) {
	static TypeId tid = ns3::TypeId("ns3::GhnPlcHelper") .SetParent<Object> ()

	.AddTraceSource("CostLog",
			"The cost at the moment of the network start up",
			MakeTraceSourceAccessor(&GhnPlcHelper::m_costTrace),
			"ns3::CostLogTrace::TracedCallback");
	return tid;
}

GhnPlcHelper::GhnPlcHelper(Ptr<const SpectrumModel> sm,
		Ptr<SpectrumValue> txPsd, PLC_NodeList& deviceNodes) :
...
void GhnPlcHelper::Setup(void) {
	...
	for (nit = m_node_list.begin(); nit != m_node_list.end(); nit++) {
		...
		//Create PHYs
		Ptr<GhnPlcPhyManagement> phyManager =
				CreateObject<GhnPlcPhyManagement> ();
		Ptr<GhnPlcDllManagement> dllManager =
				CreateObject<GhnPlcDllManagement> ();
		...
		if (m_phyTid == GhnPlcPhyPmdHalfD::GetTypeId()) {
			...
			phyManager->SetFrameSizeCallback(MakeCallback(
					&GhnPlcPhyPmdHalfD::GetFrameSize, pmd));
			phyManager->SetGatheredInfBitsCallback(MakeCallback(
					&GhnPlcPhyPmdHalfD::GetGatheredInformationBits, pmd));
			phyManager->SetTxInterfaceCallback(MakeCallback(
					&PLC_HalfDuplexOfdmPhy::GetTxInterface, pmd->GetObject<
							PLC_HalfDuplexOfdmPhy> ()));
			phyManager->SetRxInterfaceCallback(MakeCallback(
					&PLC_HalfDuplexOfdmPhy::GetRxInterface, pmd->GetObject<
							PLC_HalfDuplexOfdmPhy> ()));
			pma->SetSendCallback(MakeCallback(&GhnPlcPhyPmdHalfD::Send, pmd));
			pma->SetBandPlanType(m_bandplan);
			pma->SetGetPmcScheme(
					MakeCallback(
							&PLC_InformationRatePhy::GetPayloadModulationAndCodingScheme,
							pmd->GetObject<PLC_InformationRatePhy> ()));

			// Set the function to be called after successful packet reception by phy2
			pmd->SetReceiveSuccessCallback(MakeCallback(
					&GhnPlcPhyPcs::ReceiveSuccess, pcs));
			pmd->SetReceiveErrorCallback(MakeCallback(
					&GhnPlcPhyPcs::ReceiveFailure, pcs));

			NS_ASSERT(m_ncStats);
			pcs->TraceConnectWithoutContext("PhyRcvFailureLog", MakeCallback(
					&GhnPlcStats::NotifyPhyFailure, m_ncStats));
			pcs->TraceConnectWithoutContext("PhyRcvSuccessLog", MakeCallback(
					&GhnPlcStats::NotifyPhySuccess, m_ncStats));
			dllManager->SetPhyPcs(pcs);

			pmd->SetBandplan(m_bandplan);

			// Set modulation and coding scheme
			pmd->SetPayloadModulationAndCodingScheme(ModulationAndCodingScheme(
					QAM4, CODING_RATE_2_3, 0));

			//Set FEC block size
			phyManager->SetTxFecBlockSize(FEC_BLOCK_SIZE_540);

			//Set FEC rate
			phyManager->SetTxPayloadFecRate(ConvertPlcRateToGhnRate(
					CODING_RATE_2_3));
			...
			phyManager->SetFrameSizeCallback(MakeCallback(
					&GhnPlcPhyPmdFullD::GetFrameSize, pmd));
			phyManager->SetGatheredInfBitsCallback(MakeCallback(
					&GhnPlcPhyPmdFullD::GetGatheredInformationBits, pmd));
			phyManager->SetTxInterfaceCallback(MakeCallback(
					&PLC_FullDuplexOfdmPhy::GetTxInterface, pmd->GetObject<
							PLC_FullDuplexOfdmPhy> ()));
			phyManager->SetRxInterfaceCallback(MakeCallback(
					&PLC_FullDuplexOfdmPhy::GetRxInterface, pmd->GetObject<
							PLC_FullDuplexOfdmPhy> ()));
			pma->SetSendCallback(MakeCallback(&GhnPlcPhyPmdFullD::Send, pmd));
			pma->SetBandPlanType(m_bandplan);
			pma->SetGetPmcScheme(MakeCallback(
					&PLC_InfRateFDPhy::GetPayloadModulationAndCodingScheme,
					pmd->GetObject<PLC_InfRateFDPhy> ()));
			...
			pmd->GetObject<PLC_InfRateFDPhy> ()->SetEndRxHeaderCallback(
					MakeCallback(&GhnPlcPhyPmdFullD::EndRxHeader, pmd));
		...
		if (m_phyTid == GhnPlcPhyPmdHalfD::GetTypeId()) {
			dllManager->GetDllMac()->SetSetMcsCallback(
					MakeCallback(
							&PLC_InformationRatePhy::SetPayloadModulationAndCodingScheme,
							StaticCast<PLC_InformationRatePhy, PLC_Phy> (phy)));
			dllManager->GetDllMac()->SetSetTxPsdCallback(MakeCallback(
					&PLC_HalfDuplexOfdmPhy::SetTxPowerSpectralDensity,
					StaticCast<PLC_HalfDuplexOfdmPhy, PLC_Phy> (phy)));
		} else if (m_phyTid == GhnPlcPhyPmdFullD::GetTypeId()) {

			dllManager->GetDllMac()->SetSetMcsCallback(MakeCallback(
					&PLC_InfRateFDPhy::SetPayloadModulationAndCodingScheme,
					StaticCast<PLC_InfRateFDPhy, PLC_Phy> (phy)));
			dllManager->GetDllMac()->SetSetTxPsdCallback(MakeCallback(
					&PLC_FullDuplexOfdmPhy::SetTxPowerSpectralDensity,
			...
			StaticCast<PLC_InfRateFDPhy, PLC_Phy> (phy)->SetCollisionDetection(
					MakeCallback(&GhnPlcDllMacCsmaCd::CollisionDetection,
							(StaticCast<GhnPlcDllMacCsmaCd, GhnPlcDllMac> (
		...
		llc->SetCreateFlowCallback(
				MakeCallback(&GhnPlcHelper::CreateFlow, this));
		...
		if (m_macTid == GhnPlcDllMacCsma::GetTypeId()) {
			...
			dllManager->GetDllMac()->SetCcaRequestCallback(MakeCallback(
					&PLC_HalfDuplexOfdmPhy::CcaRequest, StaticCast<
							PLC_HalfDuplexOfdmPhy, PLC_Phy> (phy)));
			dllManager->GetDllMac()->SetCcaCancelCallback(MakeCallback(
					&PLC_HalfDuplexOfdmPhy::CancelCca, StaticCast<
							PLC_HalfDuplexOfdmPhy, PLC_Phy> (phy)));

			phy->SetFrameSentCallback(MakeCallback(
					&GhnPlcDllMac::NotifyTransmissionEnd,
					dllManager->GetDllMac()));
			StaticCast<PLC_HalfDuplexOfdmPhy, PLC_Phy> (phy)->SetCcaConfirmCallback(
					MakeCallback(&GhnPlcDllMacCsma::CcaConfirm,
							dllManager->GetDllMac()));
		} else if (m_macTid == GhnPlcDllMacCsmaCd::GetTypeId()) {
			NS_LOG_LOGIC("CSMA CD is chosen");
			StaticCast<GhnPlcDllMacCsmaCd, GhnPlcDllMacCsma> (
					dllManager->GetDllMac())->SetCcaRequestCallback(
					MakeCallback(&PLC_FullDuplexOfdmPhy::CcaRequest,
							StaticCast<PLC_FullDuplexOfdmPhy, PLC_Phy> (phy)));
			StaticCast<GhnPlcDllMacCsmaCd, GhnPlcDllMacCsma> (
					dllManager->GetDllMac())->SetCcaCancelCallback(
					MakeCallback(&PLC_FullDuplexOfdmPhy::CancelCca, StaticCast<
							PLC_FullDuplexOfdmPhy, PLC_Phy> (phy)));

			phy->SetFrameSentCallback(MakeCallback(
					&GhnPlcDllMacCsma::NotifyTransmissionEnd,
					dllManager->GetDllMac()));
			StaticCast<PLC_FullDuplexOfdmPhy, PLC_Phy> (phy)->SetCcaConfirmCallback(
					MakeCallback(&GhnPlcDllMacCsmaCd::CcaConfirm, StaticCast<
							GhnPlcDllMacCsmaCd, GhnPlcDllMacCsma> (
							dllManager->GetDllMac())));
		}
		...
		dev->SetDllApc(apc);
		dev->SetPhy(phy);
		dllManager->SetTxPsdCallback(MakeCallback(
				&PLC_NetDevice::SetTxPowerSpectralDensity, dev));

		apc->SetNetDevice(dev);
		apc->SetDllLlc(llc);
		apc->SetApduForwardDownCallback(MakeCallback(&GhnPlcDllLlc::SendFrom,
				llc));
		apc->SetAdpForwardUpCallback(MakeCallback(&GhnPlcNetDevice::Receive,
				dev));
		...
		dev->SetAddress(UanAddress::Allocate());
	...
	for (dit = m_netdeviceMap.begin(); dit != m_netdeviceMap.end(); dit++) {
		dit->second->SetVitualBroadcastCallback(MakeCallback(
				&GhnPlcHelper::VitualBroadcast, this));
	}
...
void GhnPlcHelper::CreateRoutingTable() {
...
void GhnPlcHelper::CreateBitLoadingTable() {
	...
	if (m_bitLoadingTid == NcBlVarBatMap::GetTypeId()) {
		for (nit = m_node_list.begin(); nit != m_node_list.end(); nit++) {
			m_bitLoadingTable->GetObject<NcBlVarBatMap> ()->SetPer(
					(*nit)->GetVertexId(), 0.01);
		}
	}
...
void GhnPlcHelper::SetNodeList(PLC_NodeList& deviceNodes) {
	m_node_list = deviceNodes;
}

Ptr<const SpectrumModel> GhnPlcHelper::GetSpectrumModel() {
	return m_spectrum_model;
}

void GhnPlcHelper::DefinePhyType(TypeId tid) {
	m_phyTid = tid;
}

void GhnPlcHelper::DefineMacType(TypeId tid) {
	m_macTid = tid;
}

void GhnPlcHelper::DefineBitLoadingType(TypeId id) {
	m_bitLoadingTid = id;
}
void GhnPlcHelper::SetRxImpedance(Ptr<PLC_Impedance> rxImpedance) {
...
void GhnPlcHelper::SetTxImpedance(Ptr<PLC_Impedance> txImpedance) {
...
NetDeviceContainer GhnPlcHelper::GetNetDevices(void) {
...
void GhnPlcHelper::SetChannel(Ptr<PLC_Channel> channel) {
	m_channel = channel;
}
bool GhnPlcHelper::IsCommunicationPossible(uint32_t src_id, uint32_t dst_id) {
