...
TypeId GhnPlcLlcCodedFlow::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::GhnPlcLlcCodedFlow") .SetParent<
			GhnPlcLlcFlow> () .AddConstructor<GhnPlcLlcCodedFlow> ();
	return tid;
}
...
GhnPlcLlcCodedFlow::GhnPlcLlcCodedFlow() {
	...
	m_rxSegmenter = new GhnPlcSegmenter(m_blockSize
			- header.GetSerializedSize() - GHN_CRC_LENGTH
			- sizeof(local_size_t));
	m_txSegmenter = new GhnPlcSegmenter(m_blockSize
			- header.GetSerializedSize() - GHN_CRC_LENGTH
			- sizeof(local_size_t));
...
void GhnPlcLlcCodedFlow::Configure(ncr::NodeType type, ncr::UanAddress dst) {
	m_nodeType = type;
	m_numGen = 10;
	...
	m_brr = routing_rules_ptr(new NcRoutingRules(m_id, m_nodeType, dst,
			m_numGen, m_sendRate, m_genSize, numGenBuffering));

	if (m_nodeType == SOURCE_NODE_TYPE) {
		m_encQueue = encoder_queue_ptr(new encoder_queue(m_numGen, m_genSize,
				m_symbolSize));
		m_encQueue->set_notify_callback(std::bind(
				&GhnPlcLlcCodedFlow::NotifyRcvUp, this, std::placeholders::_1));
		m_getRank = std::bind(&encoder_queue::rank, m_encQueue,
				std::placeholders::_1);
		m_brr->SetGetRankCallback(m_getRank);
		m_brr->SetHelpInfoCallback(std::bind(&encoder_queue::get_help_info,
				m_encQueue, std::placeholders::_1, std::placeholders::_2));

		SIM_LOG(COMM_NODE_LOG, "Node " << m_id << " type " << m_nodeType);
	} else {
		m_decQueue = decoder_queue_ptr(new decoder_queue(m_numGen, m_genSize,
				m_symbolSize));
		m_getRank = std::bind(&decoder_queue::rank, m_decQueue,
				std::placeholders::_1);
		m_brr->SetGetRankCallback(m_getRank);
		m_brr->SetGetCodingMatrixCallback(std::bind(
				&decoder_queue::get_coding_matrix, m_decQueue,
				std::placeholders::_1));
		m_brr->SetHelpInfoCallback(std::bind(&decoder_queue::get_help_info,
				m_decQueue, std::placeholders::_1, std::placeholders::_2));

		SIM_LOG(COMM_NODE_LOG, "Node " << m_id << " type " << m_nodeType);
	}
}

void GhnPlcLlcCodedFlow::NotifyRcvUp(GenId genId) {

	m_brr->UpdateRcvd(genId, m_id, 1);
}

SendTuple GhnPlcLlcCodedFlow::SendDown() {
	NS_LOG_FUNCTION (this << m_connId);

	if (m_connId.dst == UanAddress::GetBroadcast())
		return GhnPlcLlcFlow::SendDown();
...
auto	dll = m_dllMac->GetDllManagement ();
	...
	for(auto p : plan)
	{
		...
		for(; i < n; i++)
		{
			...
			vheader<local_msg_type>::append(contents, (local_msg_type)DATA_MSG_TYPE);
...
GhnPlcLlcFlow::Receive (GhnBuffer buffer, ConnId connId)
{
	...
	for(auto pkt : buffer)
	{
		...
		ProcessRcvdPacket(pkt, *it, header.addr, tx_it);
		it++;
	};;
...
void GhnPlcLlcCodedFlow::ProcessRcvdPacket(Ptr<Packet> pkt, bool crc, ncr::UanAddress addr, std::map<GenId,TxPlanItem>::iterator item)
{
	GenId genId = item.;
	...
	if(!crc)
	{
		...
		m_brr->UpdateLoss(genId, header.addr, 1);
		return;
	}
	auto str = ConvertPacketToStr(pkt);
	auto t = vheader<local_msg_t>::get(str);
	...
	if (t == DATA_MSG_TYPE) {
		...
		m_decQueue->enque(c, genId);
		auto rank_a = m_decQueue->rank(genId);
		if(rank_b < rank_a)
			m_brr->UpdateRcvd(genId, header.addr, 1, m_decQueue->get_uncoded());
		else
			m_brr->UpdateRcvd(genId, header.addr, 1, true);
		//
		// --->
		//
		if (m_brr->MaySendNetDiscovery()) {
			...
			if (m_brr->MaySendRetransRequest(m_decQueue->get_ranks(),
							header.addr, genId, header.all_prev_acked)) {
				SIM_LOG(COMM_NODE_LOG, "Node " << m_id
...
GhnBuffer GhnPlcLlcCodedFlow::ConvertFeedbackToBuffer(FeedbackInfo f)
{
	...
	do {
		...
		vheader<local_size_t>::append(sub_str, actual_block_size);
...
FeedbackInfo GhnPlcLlcCodedFlow::ConvertBufferToFeedback(GhnBuffer buf)
{
	...
	for(auto pkt : buf)
	{
		...
		local_size_t actual_block_size = vheader<local_size_t>::get(sub_str);
...
Ptr<Packet> GhnPlcLlcCodedFlow::ConvertStrToPacket(std::string str)
{
	return Create<Packet>(str.c_str(), str.size());
}
std::string GhnPlcLlcCodedFlow::ConvertPacketToStr(Ptr<Packet> pkt)
{
	uint8_t * v = new uint8_t[pkt->GetSize() + 1];
	pkt->CopyData(v.data(), pkt->GetSize());
	return std::string(v, pkt->GetSize());
}
//
