 ...
 *      Author: tsokalo
 */
#include "water-filling.h"
#include <iostream>
#include <assert.h>

WaterFiller::WaterFiller (std::vector<double> levels, double energy) :
...
void
WaterFiller::BubbleSort (std::vector<double> &nbrs, std::vector<uint32_t> &indexes)
{
...
double
WaterFiller::FindLevel (std::vector<double> levels, double energy)
{
...
void
WaterFiller::VisualizeLevels (std::vector<double> levels)
{
...
void
WaterFiller::VisualizeLevels (std::vector<double> levels, std::vector<double> fill)
{
  for (uint32_t i = 0; i < levels.size (); i++)
    {
      for (uint32_t j = 0; j < (uint32_t) (levels.at (i) * 10); j++)
...
void
WaterFiller::OrigReoder (std::vector<double> &nbrs, std::vector<uint32_t> &indexes)
{
  assert(nbrs.size() == indexes.size());
  uint32_t s = nbrs.size ();
