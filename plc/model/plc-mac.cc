...
//        PLC_MAC_LOGIC ("CSMA/CA backoff periods: " << backoffPeriod);
	...
	if(m_backoff_callback.IsNull())
	  ...
	  {
            backoffPeriod = m_backoff_callback();
	  }
//	randomBackoff = MicroSeconds (backoffPeriod * m_aUnitBackoffPeriod * PLC_Phy::GetSymbolDuration().GetMicroSeconds());
	//tsokalo
	if(m_aUnitBackoffPeriod != 0)
	  {
            randomBackoff = MicroSeconds (backoffPeriod * m_aUnitBackoffPeriod * PLC_Phy::GetSymbolDuration().GetMicroSeconds());
	  }
	else
	  {
            randomBackoff = backoffPeriod * m_backoffSlotDuration;
	  }

	NS_LOG_UNCOND ("Requesting CCA after backoff of " << randomBackoff);
...
void
PLC_Mac::SetState(MacState state)
{
  NS_LOG_LOGIC("Change MAC state to " << state);
  m_state = state;
}
void
PLC_Mac::StartNewCycle()
{
...
PLC_ArqMac::DoSendFrom (Ptr<Packet> p, Mac48Address src, Mac48Address dst)
{
	...
	NS_LOG_LOGIC ("PLC_ArqMac::DoSendFrom Packet size without mac header: " << txPacket->GetSize ());
...
//        m_txHeader.Print (std::cout);

        NS_LOG_LOGIC ("PLC_ArqMac::DoSendFrom Packet size with mac header: " << txPacket->GetSize ());

	if (!m_txQueue->Enqueue (Create<QueueItem>(txPacket)))
	{
...
//	StartCsmaCa();
	//tsokalo
	if(m_state == IDLE_MAC_STATE)
	  {
            SetState(WORKING_MAC_STATE);
            StartCsmaCa();
	  }
...
PLC_ArqMac::DoProcess (Ptr<const Packet> p)
{
	...
	{
		PLC_MAC_INFO ("Packet with size " << p->GetSize() << " is not for me, destination: " << dst_addr << ", my address: " << m_address << ", discarding...");
		m_rxPacket = 0;
		StartNewCycle();
	}
}
...
{
  m_phy = phy;
  if (phy->GetInstanceTypeId().IsChildOf(PLC_HalfDuplexOfdmPhy::GetTypeId()))
  {
    Ptr<PLC_HalfDuplexOfdmPhy> phy_l = StaticCast<PLC_HalfDuplexOfdmPhy, PLC_Phy> (m_phy);
    SetCcaRequestCallback(MakeCallback(&PLC_HalfDuplexOfdmPhy::CcaRequest, phy_l));
    phy_l->SetCcaConfirmCallback(MakeCallback(&PLC_Mac::CcaConfirm, this));
    phy_l->SetReceiveSuccessCallback(MakeCallback(&PLC_Mac::NotifyReceptionEndOk, this));
    phy_l->SetFrameSentCallback(MakeCallback(&PLC_Mac::NotifyTransmissionEnd, this));

  }
  else if (phy->GetInstanceTypeId().IsChildOf(PLC_FullDuplexOfdmPhy::GetTypeId()))
  {
    NS_LOG_LOGIC ("Init full duplex PHY");
    Ptr<PLC_FullDuplexOfdmPhy> phy_l = StaticCast<PLC_FullDuplexOfdmPhy, PLC_Phy> (m_phy);
    SetCcaRequestCallback(MakeCallback(&PLC_FullDuplexOfdmPhy::CcaRequest, phy_l));
    phy_l->SetCcaConfirmCallback(MakeCallback(&PLC_Mac::CcaConfirm, this));
    phy_l->SetReceiveSuccessCallback(MakeCallback(&PLC_Mac::NotifyReceptionEndOk, this));
    phy_l->SetFrameSentCallback(MakeCallback(&PLC_Mac::NotifyTransmissionEnd, this));
  }
