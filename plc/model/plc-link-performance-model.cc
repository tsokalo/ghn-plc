...
PLC_ErrorRateModel::GetBler(ModulationAndCodingScheme mcs, SpectrumValue& sinr)
{
	...
	if ((mcs.mt == BPSK) && (mcs.ct == CODING_RATE_1_4))
	  mcsnum = 0;
	else if ((mcs.mt == BPSK) && (mcs.ct == CODING_RATE_1_2))
	  mcsnum = 1;
	else if ((mcs.mt == QAM4) && (mcs.ct == CODING_RATE_1_2))
	  mcsnum = 2;
	else
	  NS_ASSERT_MSG(3 < 2, "Unsupported Modulation and Coding Scheme for PLC_ErrorRateModel!");

	double C = Sum(Log2((1 + sinr))) / sinr.GetSpectrumModel()->GetNumBands();
	double BlerApprox = pow(10, (s_A[m_channel_condition][mcsnum]*C + s_B[m_channel_condition][mcsnum]));
...
PLC_InformationRateModel::s_mcs_info[13] =
{
        {QAM, 0},       // NOMOD
        {QAM, 2},       // BPSK
        {QAM, 4},       // QAM4
