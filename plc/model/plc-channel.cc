...
PLC_ChannelTransferImpl::GetTypeId  (void)
{
	static TypeId tid = ns3::TypeId ("ns3::PLC_ChannelTransferImpl")
	    ...
	    .AddTraceSource  ("PLC_ChannelData",
					...
					"ns3::Time::PLC_TransferBaseTracedCallback")
    	    ;
	return tid;
}
...
PLC_Channel::GetTypeId  (void)
{
	static TypeId tid = ns3::TypeId ("ns3::PLC_Channel")
    	    ...
    	    .AddTraceSource ("PLC_ChannelState",
			     ...
			     "ns3::Time::IntBoolTracedCallback")
...
//	Simulator::Schedule(duration, &PLC_Channel::TransmissionEnd, this, txId, max_delay);
	//tsokalo: here nothing was changed except of that now we save info about all pending TransmissionEnd events
	for(std::vector<TransmissionEndData>::iterator e = m_transmissionEndEvent.begin(); e != m_transmissionEndEvent.end(); )
	  {
            if(e->event.IsExpired())
              m_transmissionEndEvent.erase(e);
            else
              e++;
	  }
	m_transmissionEndEvent.push_back(TransmissionEndData(Simulator::Schedule(duration, &PLC_Channel::TransmissionEnd, this, txId, max_delay), txId, max_delay));
...
PLC_Channel::TransmissionEnd (uint32_t srcId, Time propagation_delay)
{
	...
	NS_LOG_UNCOND  (this << srcId << propagation_delay);
...
//tsokalo
void PLC_Channel::CancelTransmission()
{
  for(auto e : m_transmissionEndEvent)
  {
    NS_LOG_UNCOND("Cancel event with uid " << e.event.GetUid());
    if(e.event.IsRunning())
      {
        e.event.Cancel();
        Simulator::ScheduleNow(&PLC_Channel::TransmissionEnd, this, e.srcId, e.delay);
      }
  }
  m_transmissionEndEvent.clear();
}
