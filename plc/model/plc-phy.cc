...
size_t
EncodedBits (size_t uncoded_bits, CodingType ct)
{
	return ceil(((double) uncoded_bits) / GetCodeRate(ct));
}

size_t
RequiredSymbols (size_t encoded_bits, ModulationType mt, size_t subbands)
{
	NS_LOG_LOGIC ("encoded bits: " << encoded_bits << "mct: " << mt << "subbands: " << subbands);
	size_t bits_per_subcarrier = GetBitsPerSymbol (mt);
...
Time PLC_Phy::header_symbol_duration = MicroSeconds(2240);
...
void
PLC_Phy::SetHeaderSymbolDuration(Time tSymbol)
{
        NS_LOG_FUNCTION ("Phy header symbol duration set to " << tSymbol);
        header_symbol_duration = tSymbol;
}

Time
PLC_Phy::GetHeaderSymbolDuration (void)
{
        return header_symbol_duration;
}
...
PLC_HalfDuplexOfdmPhy::SendFrame (Ptr<PLC_TrxMetaInfo> metaInfo)
{
	...
	NS_LOG_UNCOND(Simulator::Now() << " -> SendFrame on PHY. Frame duration: " << tx_duration);
...
PLC_ErrorRatePhy::DoStartTx (Ptr<const Packet> ppdu)
{
	...
	size_t encoded_bits = EncodedBits (uncoded_bits, GetModulationAndCodingScheme ().ct);
	size_t symbols = RequiredSymbols (encoded_bits, GetModulationAndCodingScheme ().mt, m_numSubcarriers);
...
PLC_ErrorRatePhy::DoStartRx (uint32_t txId, Ptr<const SpectrumValue> rxPsd, Time duration, Ptr<const PLC_TrxMetaInfo> metaInfo)
{
	...
	if (metaInfo && // meta information present
		...
		metaInfo->GetHeaderMcs().mt == GetModulationAndCodingScheme().mt && 	// same modulation and coding type (preamble detection is supposed to be always successful)
		metaInfo->GetHeaderMcs().ct == GetModulationAndCodingScheme().ct &&   // same modulation and coding type (preamble detection is supposed to be always successful)
...
{
	PLC_PHY_FUNCTION(this);
	NS_ASSERT_MSG(mcs.ct < CODING_RATE_RATELESS, "Rateless encoding not supported by PLC_ErrorRatePhy");
	m_mcs = mcs;
}
...
PLC_InformationRatePhy::DoStartTx (Ptr<const Packet> ppdu)
{
	...
	NS_LOG_INFO ("Header MCS: " << GetHeaderModulationAndCodingScheme ());
	metaInfo->SetPayloadMcs (GetPayloadModulationAndCodingScheme ());
	NS_LOG_INFO ("Payload MCS: " << GetPayloadModulationAndCodingScheme ());
...
//	NS_LOG_UNCOND(Simulator::Now () << " removing PLC preamble");
...
PLC_InformationRatePhy::StartReception (uint32_t txId, Ptr<const SpectrumValue> rxPsd, Time duration, Ptr<const PLC_TrxMetaInfo> metaInfo)
{
	...
	ModulationAndCodingScheme header_mcs = metaInfo->GetHeaderMcs ();
	PLC_PHY_INFO ("Header MCS: " << header_mcs);
...
PLC_InformationRatePhy::EndRxHeader(uint32_t txId, Ptr<const SpectrumValue> rxPsd, Ptr<const PLC_TrxMetaInfo> metaInfo)
{
	...
	if ((payload_mcs.mt == GetPayloadModulationAndCodingScheme ().mt) && (payload_mcs.ct == GetPayloadModulationAndCodingScheme ().ct))
	{
		if (m_information_rate_model->EndRx ())
		{
			...
			ModulationAndCodingScheme payload_mcs = metaInfo->GetPayloadMcs ();
			PLC_PHY_INFO ("Payload MCS: " << payload_mcs);
...
PLC_IncrementalRedundancyPhy::SendRedundancy (void)
{
	...
	size_t payload_symbols = RequiredSymbols (length*8, payload_mcs.mt, GetNumSubcarriers ());
	Time payload_duration = CalculateTransmissionDuration (payload_symbols);
...
PLC_IncrementalRedundancyPhy::StartReception (uint32_t txId, Ptr<const SpectrumValue> rxPsd, Time duration, Ptr<const PLC_TrxMetaInfo> metaInfo)
{
	...
	NS_ASSERT_MSG (metaInfo->GetPayloadMcs().ct >= CODING_RATE_RATELESS, "Packet not rateless encoded");
