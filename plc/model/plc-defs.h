...
#define CCA_NUM_SYMBOLS 2 //mudriievskyi: for broadband PLC (from 1901 standard)
...
//tsokalo
class BitAllocationTable : public  std::vector<ModulationType>
{
public:
  BitAllocationTable(){}
  BitAllocationTable(std::size_t s, ModulationType v) : std::vector<ModulationType>(s, v){}
//  BitAllocationTable& operator=(const BitAllocationTable& other) {
//
//      if (this != &other) {
//        this->clear();
...
{
public:
	ModulationAndCodingScheme (void) { mt = BPSK; ct = CODING_RATE_1_2; gap2Capacity_dB = 0; use_bat = false;}
	ModulationAndCodingScheme (const ModulationAndCodingScheme &mcs) { *this = mcs; }
	ModulationAndCodingScheme (ModulationType mtype, CodingType ctype, double g) { mt = mtype; ct = ctype; gap2Capacity_dB = g; use_bat = false;}

	//tsokalo
	ModulationAndCodingScheme& operator=(const ModulationAndCodingScheme& other) {

            if (this != &other) {
              this->mt = other.mt;
              this->ct = other.ct;
              this->gap2Capacity_dB = other.gap2Capacity_dB;
              this->use_bat = other.use_bat;
              this->bat = other.bat;
            }
            return *this;
          }
	...
	bool use_bat;
	//tsokalo
	BitAllocationTable bat;
};

std::ostream& operator<<(std::ostream& os, ModulationType mt);
std::ostream& operator<<(std::ostream& os, CodingType ct);
std::ostream& operator<<(std::ostream& os, ModulationAndCodingScheme mcs);

double GetCodeRate (CodingType ct);
size_t GetBitsPerSymbol (ModulationType mt);
