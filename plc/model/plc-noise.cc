...
PLC_TimeVaryingNoiseSource::PLC_TimeVaryingNoiseSource (Ptr<PLC_Node> src_node, Ptr<SpectrumModel> sm, Ptr<RandomVariableStream> subChannelPsd, Ptr<RandomVariableStream> staticDuration)
...
{
	PLC_LOG_FUNCTION (this);
	m_pulse_len = CreateObject<UniformRandomVariable> ();
	m_pulse_len->SetAttribute ("Min", DoubleValue (0));
	m_pulse_len->SetAttribute ("Max", DoubleValue (MAX_PULSE_NOISE_DURATION));
	m_pulse_gap = CreateObject<UniformRandomVariable> ();
	m_pulse_gap->SetAttribute ("Min", DoubleValue (0));
	m_pulse_gap->SetAttribute ("Max", DoubleValue (MAX_PULSE_GAP_DURATION));
...
PLC_ImpulsiveNoiseSource::PLC_ImpulsiveNoiseSource (Ptr<PLC_Node> src_node, Ptr<SpectrumValue> noisePsd, Ptr<RandomVariableStream> pulse_len_gen, Ptr<RandomVariableStream> pulse_gap_gen)
