...
PLC_NetDeviceHelper::Setup(void)
{
	...
	for (nit = m_netdevice_nodes.begin(); nit != m_netdevice_nodes.end(); nit++)
	{
		...
		{
			NS_ASSERT_MSG (m_header_mcs.mt == m_payload_mcs.mt, "Header and payload modulation types have to be identical for PLC_ErrorRatePhy");
                        NS_ASSERT_MSG (m_header_mcs.ct == m_payload_mcs.ct, "Header and payload coding types have to be identical for PLC_ErrorRatePhy");
			NS_ASSERT_MSG (m_header_mcs.ct < CODING_RATE_RATELESS, "Rateless encoding not supported by PLC_ErrorRatePhy");
				 ...
				 )
		{
			...
			(StaticCast<PLC_InformationRatePhy, PLC_Phy> (phy))->SetPayloadModulationAndCodingScheme(m_payload_mcs);
		        ...
		        phy->GetInstanceTypeId() == PLC_InfRateFDPhy::GetTypeId () ||
		        phy->GetInstanceTypeId().IsChildOf(PLC_InfRateFDPhy::GetTypeId ())
		)
		  {
